    ...
    public function questionnaire(int $auditId): array
    {
        $statement = $this->db->prepare(
            'SELECT
                ...
                c.id AS control_id, c.code AS control_code, c.title AS control_title,
                c.weight AS control_weight, c.confidentiality, c.integrity, c.availability,
                q.id AS question_id, q.question AS question_text, q.weight AS question_weight,
                r.id AS response_id, r.answer, r.observation, r.recommendation
             FROM iso_controls c
