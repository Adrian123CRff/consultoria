...
**Opción B — WAMP/XAMPP:**
Acceder directamente desde `http://localhost/consultora/public/` o configurar un VirtualHost.

**Opción C — VirtualHost Apache (recomendado):**
```apache
<VirtualHost *:80>
    ServerName consultora.local
    DocumentRoot "C:/wamp64/www/consultora/public"
    <Directory "C:/wamp64/www/consultora/public">
        AllowOverride All
        Require all granted
    </Directory>
</VirtualHost>
...
El archivo `public/.htaccess` ya está configurado para redirigir todas las peticiones al front controller.
...
├── sql/
│   ├── schema.sql         # Script principal de creación de BD
│   └── schema_merged.sql  # Script consolidado con migraciones
├── storage/evidences/     # Almacenamiento alternativo de evidencias
...
- **Subida de archivos:** se valida tipo MIME y se almacena con nombre generado (no el original).
...
Si se parte de una instalación anterior, aplicar los scripts en orden:

```bash
mysql -u root -p consultora_iso27002 < database/migration_fase3_4.sql
mysql -u root -p consultora_iso27002 < database/migration_fase5_6.sql
mysql -u root -p consultora_iso27002 < database/migration_fase5_6_fix.sql
mysql -u root -p consultora_iso27002 < database/migration_valor_agregado.sql
```

Para instalaciones nuevas, usar únicamente `sql/schema.sql` o `sql/schema_merged.sql`.
